...
/**
 * All tests in here use the lru memory cache
 */
...
test("Return headers works with caching client and fetchFirst", async t => {
    ...
    const client = await createFetchClient({fetch: fetch, cache: {id: "memorylru", maxItems: 5}});
    const cacheRequestConfig = {useCache: {mode: "fetchFirst", table: "Test", key: "a"}};
...
test("cloneItems works with lru cache", async t => {
    ...
    const client = await createFetchClient({
        cache: {id: "memorylru", maxItems: 5, cloneItems: true},
        fetch: fetch
    });
    const cacheRequestConfig = {useCache: {mode: "cacheControl", table: "Test", key: "a1", defaultCacheControl: {maxAge: true}}};
    const result1 = await client.fetchJson("", cacheRequestConfig);
